...
import { getCurrentSeason } from '@/styles/seasonal';
...
const StyledWrapper = styled.div<{ $palette: any }>`
  ...
  @keyframes springGlow {
    0%, 100% { box-shadow: 0 4px 12px ${props => props.$palette.shadow}, inset 0 0 20px ${props => props.$palette.shadow}; }
    50% { box-shadow: 0 6px 16px ${props => props.$palette.shadow}, inset 0 0 30px ${props => props.$palette.shadow}; }
  }

  @keyframes summerGlow {
    0%, 100% { box-shadow: 0 4px 12px ${props => props.$palette.shadow}, inset 0 0 15px ${props => props.$palette.shadow}; }
    50% { box-shadow: 0 8px 20px ${props => props.$palette.shadow}, inset 0 0 25px ${props => props.$palette.shadow}; }
  }
  ...
  @keyframes winterGlow {
    0%, 100% { box-shadow: 0 4px 12px ${props => props.$palette.shadow}, inset 0 0 15px ${props => props.$palette.shadow}; }
    50% { box-shadow: 0 6px 18px ${props => props.$palette.shadow}, inset 0 0 25px ${props => props.$palette.shadow}; }
  }
...
const SeasonLabel = styled.div<{ $season: string }>`
  ...
  background: ${props => {
    switch (props.$season) {
      case 'spring': return 'rgba(168, 216, 111, 0.2)';
      case 'summer': return 'rgba(255, 215, 0, 0.2)';
      case 'fall': return 'rgba(232, 148, 74, 0.2)';
      case 'winter': return 'rgba(176, 216, 241, 0.2)';
  ...
  color: ${props => {
    switch (props.$season) {
      case 'spring': return '#6B9E3F';
      case 'summer': return '#CC8800';
      case 'fall': return '#A85C2D';
      case 'winter': return '#5B7FA6';
      default: return '#666';
    }
  }};
`;
 ...
 */
export const SeasonalCalendarWrapper: React.FC<SeasonalCalendarWrapperProps> = ({
  children,
  className,
}) => {
